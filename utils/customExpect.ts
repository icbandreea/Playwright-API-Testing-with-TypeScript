...
//*
    // Schema Assertion recap
    // we created a custom assertion shouldMatchSchema, where we call validateSchema method that we implemented in schema-validator.ts 
    // this custom assertion should be async because validateSchema has await, that is why in the interface when 
    // declaring the method we returned it as a promise 
    // the first argument in this custom assertion is received, which is the object that is coming from our test as a argument inside of the expect 
    // and the other 2 arguments are directory name and file name to pass them into validateSchema (which returns the error message and API logs)